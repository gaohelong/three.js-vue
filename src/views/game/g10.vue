...
<script>
  ...
  let rockerGroup, rockerCircle2 // 摇杆
  ...
  export default {
    ...
    methods: {
      ...
      rockerMove(type, val) {
        if (type === 'up') { // 上
          rockerGroup.translateZ(val) // 相对平移
          rockerCircle2.position.y = 5.89 // 固定位置
        } else if (type === 'down') { // 后
          rockerGroup.translateZ(val)
