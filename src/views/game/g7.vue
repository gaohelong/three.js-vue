<template>
  <div class="container">
    ...
    <div class="rocker-wrap">
      <div :class="'rocker ' + directionCls"></div>
    </div>
  </div>
</template>
...
<script>
  ...
  // let cameraY = 36
  const cameraZMin = 3000 // 前进最大值
  const cameraZMax = 10000 // 后退最大值
  let cameraZ = 7000
  let cameraUpZ = 0
  let cameraDownZ = 0
  const cameraRightDegMax = -Math.PI / 4 // 限制右侧移动角度
  const cameraLeftDegMax = Math.PI / 4 // 限制左侧移动家都

  export default {
    name: 'game-6',
    data() {
      return {
        directionCls: 'direction-def'
      }
    },
    methods: {
      /**
      ...
      positionMove(type) {
        if (type === 'touchstart') { // 触摸开始
          ...
          if (Math.abs(X) > Math.abs(Y) && X > 0) { // 右
            this.directionCls = 'direction-right'
            ...
            this.directionCls = 'direction-left'
            ...
            this.directionCls = 'direction-down'
            if (cameraZ < cameraZMax) {
              cameraZ = cameraZ + 100
              camera.position.z = cameraZ // 设置相机Z轴位置位置
              // camera.lookAt(scene.position)
              camera.updateProjectionMatrix() // 更新摄像机投影矩阵
              // console.log(cameraZ, cameraZMax)
            }
          } else if (Math.abs(Y) > Math.abs(X) && Y < 0) { // 前
            this.directionCls = 'direction-up'
            if (cameraZ > cameraZMin) {
              cameraZ = cameraZ - 100
              camera.position.z = cameraZ // 设置相机Z轴位置位置
              // camera.lookAt(scene.position)
              camera.updateProjectionMatrix() // 更新摄像机投影矩阵
              // console.log(cameraZ, cameraZMin)
            }
          }
    ...
    mounted() {
      window.document.title = '3D展厅-方向指示器'
      this.init()
    }
  }
</script>
